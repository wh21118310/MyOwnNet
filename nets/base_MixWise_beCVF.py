...
from nets.backbone.CVF_denoising import GenNoise, GenClean
...
class CVF_model(nn.Module):
    def __init__(self, n_channels, FSize=64, clear_Layers=17, noise_Layers=10):
        super().__init__()
        self.gen_noise = GenNoise(Fsize=FSize, NLayer=noise_Layers, channels=n_channels)
        self.genclean = GenClean(channels=n_channels, numLayers=clear_Layers, features=FSize)
...
class PFNet(nn.Module):
    def __init__(self, model_path=None, bk='resnet50'):
        ...
        # Denoise Module
        self.denoise = CVF_model(n_channels=3, FSize=32, clear_Layers=12, noise_Layers=10)
        # params
        self.model = bk
        # backbone
        self.backbone = Backbone(bk=bk, model_path=model_path, in_channels=3)
    ...
    def forward(self, x):
        noise_w, noise_b, clear = self.denoise(x)
        # x: [batch_size, channel=3, h, w]
        layer1, layer2, layer3, layer4 = self.backbone(clear, self.model)
