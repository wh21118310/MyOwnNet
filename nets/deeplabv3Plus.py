...
class MobileNetv2(nn.Module):
    def __init__(self, downsample_factor=8, pretrained=False):
        ...
        '''
        膨胀系数为8，对倒数两次下采样进行修改，使其步长变为1，从而实现3次采样
        '''
    ...
    def forward(self, x):
        low_level_features = self.features[:4](x)  # 提取底层特征
        x = self.features[4:](low_level_features)  # 继续卷积
...
class ASPP(nn.Module):
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        # self.branch5_conv = nn.Conv2d(dim_in, dim_out, 1, 1)
